...
const ProductCard = ({ product }) => {
  ...
  const validatedDiscountPercentage = parseFloat(disCountPercentage);

  // Validate the parsed numbers
  const finalPrice = isNaN(validatedPrice) ? 0 : validatedPrice;
  const finalDiscountPercentage = isNaN(validatedDiscountPercentage)
    ? 0
